...
3. **Joints / Driving** — wheel/suspension + heightfield.
4. **Bodies / Cast** — body-level cast/overlap APIs still `🚧`.
5. **Character / CapsulePlane** — mover collide APIs still `🚧`.
...
| **Gyroscopic Precession** | [x] | Hull tops + `bodyDef.allowFastRotation` | 🔧 8×8 tip-balanced 7-gon hull tops; tilt 15° about Z, spin about symmetry axis via `rotateVector`, `allowFastRotation`. Yellow tip axis + spin/tilt HUD. C++/WASM dump parity verified at epsilon=1e-5. |
...
| **Wave Pile** | [x] | Shared `CreateWavePile` scenario + sleep hash | 🔧 100 mixed sphere/capsule/box/rock bodies on `b3CreateWave` heightfield; seed 52977 via WASM Random, RR=0.3. Custom worker/dumpCreate skip default ground and keep heightfield lifetime explicit. C++/WASM dump parity verified. |
| **Query Spawn** | [x] | Spawn + ray/overlap/sphere-cast queries + hashes | 🔧 Zero-g spawn every 10 steps (`frameCount % 10 == 1`); seed 71689. Host uses 50 placeholder spheres; worker grows tracked bodies. Dump uses `dumpPostStep` to match C++ post-`Sample::Step` spawn. |
| **Mesh Drop** | [x] | Shared `CreateMeshDrop` | 🔧 Wave mesh 40×40 + 20×20 boxes (seed 3963634789, filter cat=2/mask=1). Dump alias `Determinism Mesh Drop` (name collides with Continuous). Shared builder `mesh-drop-shared.ts`. |
...
| **Wheel** | [x] | `b3CreateWheelJoint` | 🔧 Empty static joint ground + dynamic cylinder (`createCylinder(0.25,0.4,0,12)`); body rot `computeQuatBetweenUnitVectors(Y,Z)`; wheel joint frames X→Y / Z→Y, `collideConnected`, suspension spring off (`hertz=2`, damping 0.7), limits/motors off. C++/WASM dump parity verified at epsilon=1e-5. |
...
| **Height Field** | [x] | `b3CreateWave`, `b3CreateHeightFieldShape` | 🔧 NDEBUG 400×400 wave (`amplitude=0.75`, holes=false); dynamics `#if 0` disabled. Host heightfield overlay; dumpCreate disposes heightfield. Dump C++ name is `Mesh/Height Field` (disambiguates Benchmark / Height Field). |
...
| **Pile** | [x] | Multiple humans piling | 🔧 20 humans on mesh floor (release build count). Upstream seeds `g_randomSeed=42` and places each human via `RandomVec3` at y=2 (`Box3DRng(42)` with float32-safe `RandomFloatRange`). C++/WASM dump parity verified at epsilon=1e-5 on default checkpoints (0,50,100,200,300). |
...
| **Card House** | [x] | Thin card stacking (tiny thickness hull) | 🔧 Exact C++ layout; includes upstream `rollingResistance=0.05` + `friction=0.7`. Float32-safe roof Y (`y + cardHeight - 0.015f`) matches upstream; default dump checkpoints `0,50,100,200,300` are bit-exact (`epsilon=0`). |
...
| **Far Mesh Drop** | [x] | Shared `CreateMeshDrop` at far origin | 🔧 Same as Determinism Mesh Drop with origin `{1e6,0,1e6}`. Dump `Far Mesh Drop`. |

## Benchmark (`sample_benchmark.cpp`)

| Sample | TS | APIs needed | Notes |
|--------|----|-------------|-------|
| **Large Pyramid** | [x] | Many box pyramid | 🔧 Release upstream `CreateLargePyramid` baseCount **100** (5050 boxes) via `createHullShape`/`b3MakeBoxHull` (not QHull-from-points). Continuous stays world default (on); dump create disables sleep. Shared multi-layer `shader-instanced-host` with awake/sleep debug colors. C++/WASM dump parity verified at epsilon=1e-5 on default checkpoints. |
...
| **Large World** | [x] | Hull `invokeContactCreation`, capacity | 🔧 Live demo: **200×200** floor tiles (40k statics, cell 10) + 100 spheres every 5 steps — halfway under `B3W_MAX_*=65536` (release C++ is 1000×1000 / 100). Dump stays **debug 32×32 / 16 spheres / every 8** to match `DumpLargeWorld`. Host: instanced tile floor + shader-instanced spheres. |
...
| **Convex Pile** | [x] | Shared `CreateConvexPile` (fixed LCG hull) | 🔧 Release 8×8×80 = 5120 convexes from 32 sphere points (seed 42 PEEL LCG); ground 250×1×250; world capacity prealloc. `shader-instanced-host` + `ConvexGeometry`. Frames 0–50 match at 1e-5; soft multi-contact drift by ~frame 75–100 (tracked in `scripts/dump-soft-exceptions.json`). |
...
- **TS implemented (matching C++)**: **118** (~74.2%)
