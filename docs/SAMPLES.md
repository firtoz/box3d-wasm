...
| **Mesh Drop** | [ ] | Mesh + CCD | 🧩 Upstream seeds from `b3GetTicks()` each `Generate()` — not dump-comparable without a fixed-seed dump override. |
| **Hump Mesh** | [ ] | Mesh + CCD | 🧩 |
...
| **GMod Wheel Stack** | [x] | Multi-piece wheel hulls + `SetContactTuning` | 🔧 30 stacked wheels from `metal_wheel1` verts (wrapping convex hull; multi-piece shapes commented out upstream). `setContactTuning(240,10,3)`. Setup bit-exact (frame 0); ~1 ULP at frame 1; soft multi-contact drift from ~frame 2 (same class as other stacks — default checkpoints diverge). |
...
| **Convex Pile** | [x] | Shared `CreateConvexPile` (fixed LCG hull) | 🔧 Release 8×8×80 = 5120 convexes from 32 sphere points (seed 42 PEEL LCG); ground 250×1×250; world capacity prealloc. `shader-instanced-host` + `ConvexGeometry`. Setup + early sim bit-exact through frame 40 at 1e-5; soft multi-contact drift by default frame-50 checkpoint (same class as Junkyard / Candy Cups piles). |
...
- **Dump-match status**: After the `box3d` bump to `3fc20f5`, a full dump-enabled sweep (99 scenes, default frames/`1e-5`) passed **93** once Jenga + Large Pyramid setup bugs were fixed (were 91/99; those two were pre-existing body-count mismatches, not bump regressions). Remaining soft exceptions are multi-contact / long-horizon FP drift, not missed scene parameters: `stacking/card-house` (thin cards diverge from ~frame 50 after upstream SIMD hull collision; thick extra still matches), `world/far-ragdolls` (matches through ~frame 29), `determinism/falling-ragdolls` (matches through frame 200; frame 300 drifts), `shapes/wind-flap` (frames 0–200 exact; ~1–3e-5 drift at frame 300), `benchmark/candy-cups` (scripted explode at frame 200; frames 0–200 match including post-blast; frame 300 drifts), `benchmark/explosion` (scripted explode at frame 1; frames 0–20 match; drift from ~frame 30), `issues/gmod-wheel-stack` (setup bit-exact; soft drift from ~frame 2), `benchmark/convex-pile` (matches through frame 40; soft drift by frame 50), `benchmark/junkyard` (frames 0–100 match; drift ~frame 130), `benchmark/large-pyramid` (Release baseCount 100; frame 0 exact; soft drift from ~frame 10). `benchmark/destruction` matches default checkpoints (dump skips mid-run respawn). Native SIMD vs scalar dumps are bit-identical; WASM SIMD vs scalar dumps are bit-identical — residual drift is cross-target codegen/libm, not SSE vs wasm SIMD128. `geometry/hull` uses `dumpNoPhysics` (upstream sample has no bodies).
