...
- **Samples**: 108 C++ sample scenes currently ported to TypeScript (~159 unique upstream `RegisterSample`s), with a tracking document for the remaining samples
...
- **WASM size**: **~247KB gzipped (581KB raw)** for the release binary, built with `-O3`, pthreads, and WASM SIMD enabled
...
| Samples/examples | 28+ API-focused examples | Custom Three.js demo scenes | 108/159 C++ samples ported | All ~159 native samples |
...
| WASM size | ~309KB gzipped (806KB raw) | ~211KB gzipped (521KB raw) standard; ~218KB gzipped (533KB raw) deluxe | ~247KB gzipped (581KB raw) | Large app binary, fixed 512MB heap |
...
| WASM gzipped | ~309KB | ~211KB standard; ~218KB deluxe | ~247KB | N/A |
