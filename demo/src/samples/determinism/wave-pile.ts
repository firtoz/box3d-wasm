...
import { f32 } from "../f32";
import {
  createWavePileBodies,
  wavePileCamera,
  wavePileGroundSize,
  wavePileHeightFieldVisual,
} from "./wave-pile-scene";

function createWaveHeightFieldVisual(scene: THREE.Scene): THREE.Group {
  const { rowCount, columnCount, scale, rowFrequency, columnFrequency, position } = wavePileHeightFieldVisual;
...
const half = wavePileGroundSize();

export const wavePileSample: DemoSample = {
  id: "determinism/wave-pile",
  name: "Determinism / Wave Pile",
  create(runtime, scene, solverParams) {
    ...
    const spec: RenderSpec = {
      ...
      groundKind: "none",
      bodies: createWavePileBodies(),
      camera: wavePileCamera,
      info: "100 mixed convex bodies on wave heightfield (CreateWavePile)",
    ...
    return createGenericSample(
      "determinism/wave-pile",
      "Determinism / Wave Pile",
      spec,
      () => new Worker(new URL("./wave-pile.worker.ts", import.meta.url), { type: "module" }),
    ).create(runtime, scene, solverParams);
  },
};
