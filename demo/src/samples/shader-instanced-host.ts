...
export type ShaderInstancedHostSpec = {
  ...
  profile?: boolean;
  info: string | ((ctx: { workerCount: number; colorMode: "light" | "full" }) => string);
  shape:
...
export function createShaderInstancedSample(spec: ShaderInstancedHostSpec): DemoSample {
  ...
  return {
    ...
    create(_runtime: Box3DRuntime, scene: THREE.Scene, initialSolverParams?: SolverParams) {
      ...
      let bodyCount = spec.instanceCount;
      ...
      let shaderMesh: ShaderBoxMesh;
      if (spec.shape.kind === "sphere") {
        const base = new THREE.SphereGeometry(
          spec.shape.radius,
          spec.shape.widthSegments ?? 12,
          spec.shape.heightSegments ?? 8,
        );
        shaderMesh = createShaderInstanceMesh(base, spec.instanceCount, { shadows: false });
        base.dispose();
      } else {
        shaderMesh = createShaderBoxMesh(spec.instanceCount, spec.shape.size ?? 1, { shadows: false });
      }

      let idx = 0;
      spec.forEachInstance((position, color) => {
        if (idx >= spec.instanceCount) return;
        const pOff = idx * 3;
        const qOff = idx * 4;
        shaderMesh.positionArray[pOff] = position[0];
        shaderMesh.positionArray[pOff + 1] = position[1];
        shaderMesh.positionArray[pOff + 2] = position[2];
        ...
        hexToRgb(color ?? defaultColor, shaderMesh.colorArray, pOff);
        idx++;
      });
      ...
      worker.addEventListener("message", (event: MessageEvent<PhysicsWorkerMessage>) => {
        ...
        if (message.type === "ready") {
          ...
          colorCache = new Uint32Array(spec.instanceCount);
          bindSnapshotTransforms(shaderMesh, positions, rotations, Math.min(spec.instanceCount, bodyCount));
          shaderMesh.positionAttribute.needsUpdate = true;
          shaderMesh.quaternionAttribute.needsUpdate = true;
      ...
      const solverParams = spec.initSolverParams !== undefined
        ? spec.initSolverParams(initialSolverParams)
        : initialSolverParams;
      worker.postMessage({
        type: "init",
        data: {},
      ...
      function step(): void {
        ...
        const count = Math.min(spec.instanceCount, bodyCount);
      ...
      const infoText = typeof spec.info === "function"
        ? spec.info({ workerCount: wc, colorMode })
        : spec.info;
      ...
      return {
        ...
        sendSolverParams(params) {
          const mapped = spec.mapSolverParams !== undefined ? spec.mapSolverParams(params) : params;
