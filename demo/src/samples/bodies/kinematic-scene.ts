import {B3_AXIS_Z, BodyType, type BodyId, type Box3DRuntime, type PhysicsWorld, type Vec3} from "box3d-wasm";
...
export function buildKinematicDynamicBodies(world: PhysicsWorld, runtime: Box3DRuntime): BodyId[] {
...
export function stepKinematic(_world: PhysicsWorld, runtime: Box3DRuntime, bodyId: BodyId, time: number, dt: number): number {
  if (dt > 0 && time > DELAY) {
    const t = time - DELAY;
    const point: Vec3 = [
...
let _kt = 0;
export function dumpStep(world: PhysicsWorld, runtime: Box3DRuntime, handles: readonly BodyId[], _frame: number, dt: number): void {
