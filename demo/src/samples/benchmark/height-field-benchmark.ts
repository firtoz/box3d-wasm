...
import { f32 } from "../f32";
import {
  benchmarkHeightFieldBodies,
  benchmarkHeightFieldCamera,
  benchmarkHeightFieldGroundSize,
  benchmarkHeightFieldVisual,
} from "./height-field-scene";

function createWaveHeightFieldVisual(scene: THREE.Scene): THREE.Group {
  const { rowCount, columnCount, scale, rowFrequency, columnFrequency, position } = benchmarkHeightFieldVisual;
...
const half = benchmarkHeightFieldGroundSize();
...
export const benchmarkHeightFieldSample: DemoSample = {
  ...
  create(runtime, scene, solverParams) {
    ...
    const spec: RenderSpec = {
      ...
      groundKind: "none",
      bodies: benchmarkHeightFieldBodies,
      camera: benchmarkHeightFieldCamera,
      info: "50×50 wave heightfield benchmark ground",
    ...
    return createGenericSample(
      "benchmark/height-field",
      "Benchmark / Height Field",
      spec,
      () => new Worker(new URL("./height-field-benchmark.worker.ts", import.meta.url), { type: "module" }),
    ).create(runtime, scene, solverParams);
  },
};
