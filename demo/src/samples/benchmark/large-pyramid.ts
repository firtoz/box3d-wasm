...
export const largePyramidSample: DemoSample = {
  ...
  create(_runtime: Box3DRuntime, scene: THREE.Scene, initialSolverParams?: SolverParams) {
    ...
    const worker = new Worker(new URL("./large-pyramid.worker.ts", import.meta.url), { type: "module" });
    ...
    const shaderMesh = createShaderBoxMesh(LARGE_PYRAMID_BOX_COUNT, 1, { shadows: false });
    let idx = 0;
    forEachLargePyramidBox(([x, y, z]) => {
      const pOff = idx * 3;
      const qOff = idx * 4;
      ...
      hexToRgb(LARGE_PYRAMID_BOX_COLOR, shaderMesh.colorArray, pOff);
      idx++;
    });
