...
export function createBenchmarkTileMeshes(world: PhysicsWorld) {
  ...
  const meshGridCellWidth = f32(gridSize / f32(2 * halfMeshGridRows));
  const gridMesh = world.createGridMesh(8, 8, meshGridCellWidth, 1, true);
  const torusMesh = world.createTorusMesh(16, 16, f32(0.25 * gridSize), 1);
...
export function buildBenchmarkTileGround(world: PhysicsWorld, gridCount: number): number[] {
  ...
  const span = f32(gridSize * gridCount);
...
export function spawnHumanGroup(
  ...
  const span = f32(gridCount * gridSize);
  const groupDistance = f32(span / gridCount);
  let x = f32(f32(-0.5) * span + f32(groupDistance * f32(columnIndex + 0.5)));
  const z = f32(f32(-0.5) * span + f32(groupDistance * f32(rowIndex + 0.5)));
