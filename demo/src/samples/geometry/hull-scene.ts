...
function makeHullPoints(): number[] {
  const points: number[] = [];
  for (const p of rawPoints) {
    points.push(p[0] * 0.01, p[1] * 0.01, p[2] * 0.01);
  }
  return points;
}

export function buildHullDynamicBodies(world: PhysicsWorld, runtime: Box3DRuntime): BodyId[] {
...
export function hullGroundSize(): Vec3 {
  return [10, 1, 10];
}

export const hullBodies: RenderBody[] = [
  { kind: "box", size: [0.5, 0.5, 0.5], position: [0, 1, 0], color: 0x3b82f6 },
];

export const hullCamera: RenderSpec["camera"] = { position: [0, 15, 5], target: [0, 0, 0] };
