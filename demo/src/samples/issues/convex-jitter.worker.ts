...
class ConvexJitterWorker extends PhysicsWorkerBase {
  ...
  protected async buildScene(): Promise<number[]> {
...

    {
      ...
      const points = scalePoints(raw, s);
      const hull = this.runtime!.createHullFromPoints(points);
      const body = this.world!.createBody({
        type: BodyType.Static, position: [-0.6, 2, 0.27],
...

    {
      ...
      const hull = this.runtime!.createHullFromPoints(points);
      const body = this.world!.createBody({
        type: BodyType.Dynamic, position: [0.4, 2, 0.16],
        rotation: [0, -0.00152086187, 0, 0.999998868],
      });
      this.runtime!.createShapeFromHull(body, hull, { friction: 0.5, restitution: 0, rollingResistance: 0.1 });
      this.runtime!.destroyHull(hull);
      handles.push(body);
    }
