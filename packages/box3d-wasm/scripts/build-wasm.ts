...
await run(['bash', '-lc', `${emcmakeCommand} cmake -S ${join(packageRoot, 'cmake')} -B ${join(packageRoot, 'build-web')} -DBOX3D_SOURCE_DIR=${box3dSourceDir} -DBOX3D_SAMPLES=OFF -DBOX3D_BENCHMARKS=OFF -DBOX3D_DOCS=OFF -DBOX3D_UNIT_TESTS=OFF -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=${generatedDir} && cmake --build ${join(packageRoot, 'build-web')}`], 'Install Emscripten and ensure emcmake is available.');
